...
Module Info.
  ...
  Definition source_file := "src/binary_search.c"%string.
...
Definition _binary_search : ident := 60%positive.
Definition _four : ident := 61%positive.
Definition _high : ident := 57%positive.
Definition _key : ident := 55%positive.
Definition _len : ident := 54%positive.
Definition _low : ident := 56%positive.
Definition _main : ident := 63%positive.
Definition _mid : ident := 58%positive.
Definition _mid_val : ident := 59%positive.
Definition _s : ident := 62%positive.
Definition _t'1 : ident := 64%positive.

Definition f_binary_search := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_a, (tptr tint)) :: (_len, tint) :: (_key, tint) :: nil);
  fn_vars := nil;
  fn_temps := ((_low, tint) :: (_high, tint) :: (_mid, tint) ::
               (_mid_val, tint) :: nil);
  fn_body :=
(Ssequence
  (Sset _low (Econst_int (Int.repr 0) tint))
  (Ssequence
    (Sset _high
      (Ebinop Osub (Etempvar _len tint) (Econst_int (Int.repr 1) tint) tint))
    (Ssequence
      (Swhile
        (Ebinop Ole (Etempvar _low tint) (Etempvar _high tint) tint)
        (Ssequence
          (Sset _mid
            (Ebinop Oshr
              (Ebinop Oadd (Ecast (Etempvar _low tint) tuint)
                (Ecast (Etempvar _high tint) tuint) tuint)
            ...
            (Sset _mid_val
              (Ederef
                (Ebinop Oadd (Etempvar _a (tptr tint)) (Etempvar _mid tint)
                  (tptr tint)) tint))
            (Sifthenelse (Ebinop Olt (Etempvar _mid_val tint)
                           (Etempvar _key tint) tint)
              (Sset _low
                (Ebinop Oadd (Etempvar _mid tint)
                  (Econst_int (Int.repr 1) tint) tint))
              (Sifthenelse (Ebinop Ogt (Etempvar _mid_val tint)
                             (Etempvar _key tint) tint)
                (Sset _high
                  (Ebinop Osub (Etempvar _mid tint)
                    (Econst_int (Int.repr 1) tint) tint))
                (Sreturn (Some (Etempvar _mid tint))))))))
      (Sreturn (Some (Ebinop Osub (Eunop Oneg (Etempvar _low tint) tint)
                       (Econst_int (Int.repr 1) tint) tint))))))
|}.
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_binary_search, Gfun(Internal f_binary_search)) :: (_four, Gvar v_four) ::
 (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _four :: _binary_search :: ___builtin_debug ::
